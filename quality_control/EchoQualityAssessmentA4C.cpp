...
EchoQualityAssessmentA4C::EchoQualityAssessmentA4C(EchoQualityAssessmentParams params)
{
    // -------------------------------- 先验知识参数初始化 -------------------------------- //
    this->mParams = &params;
    // 切面类型名称
    ...
    this->structure_score_structures.routine_must = { "LA","LV","RA","RV","IAS","IVS" };  // 常规必须项结构
    this->structure_score_structures.routine_optional = {};                               // 常规可选项结构
    this->structure_score_structures.unroutine_optional = { "MV","TV" };                  // 非常规项结构
    this->structure_score_structures.unroutine_optional_params = { 0.70f, 0.4f };      // 非常规项参数
    this->structure_score_weight.routine_must = 4.0f;                               // 常规必须项结构分数权重
    this->structure_score_weight.routine_optional = {};                             // 常规可选项结构分数权重
    this->structure_score_weight.unroutine_optional = { 1.0f, 1.0f };               // 非常规项结构分数权重
...
EchoQualityAssessmentA4C::~EchoQualityAssessmentA4C()
{
...
void
EchoQualityAssessmentA4C::load_gain_model()
{
    //this->mParams->gain_params.viewname = "a4c";
    //this->mParams->gain_params.view_structures = this->structures;
    //this->mParams->gain_params.gain_structures = { "RV" };
...
void
EchoQualityAssessmentA4C::load_structure_model()
{
    //this->mParams->sdetection_params.viewname = "a4c";
    //this->mParams->sdetection_params.structures = this->structures;
    //this->mParams->sdetection_params.classnames = this->classnames;
...
float
EchoQualityAssessmentA4C::scaling_judgment(s_frames structure_frames,
...
float EchoQualityAssessmentA4C::structure_judgment(s_frames structure_frames,
    int frame_nums,
    std::vector<float> &grades)
{
    ...
    for (auto it : structure_frames) {
        ...
        for (int i = 0; i < frame_nums; i++) {
            int temp = get_vector_idx_int(it.second, i)>0 ? 1: 0; //在vector中找到对应对象的索引（取最小值）
    ...
    if (!routine_optional.empty())
    {
        for (int i = 0; i < routine_optional.size(); i++)
        {
            ...
            int structure_idx = this->structure_idx_mapping[structure];// 获取当前常规结构的索引
            int temp = int(std::accumulate(structure_preds[structure].begin(), structure_preds[structure].end(), 0.0));
            float pred_structure = temp >= this->frame_threshold[structure_idx] ? 1.0 : 0.0;
    ...
    if (!unroutine_optional.empty())
    {
        for (int i = 0; i < unroutine_optional.size(); i++)
        {
            ...
            //类中的一个嵌套类型，表示该迭代器是一个常量迭代器，它不允许修改所指向元素的值。
            ...
            {
                pred_structure = 1.0;
 //               std::cout << "pred_structure = 1.0" << std::endl;
            }
            ...
            /*if (keyframes.size()==0) {
                break;
                std::vector<int> subpreds;
                subpreds.assign(preds_first , preds_first + keyframes[0]);
    ...
    std::cout << "this video structure_score : " << this->structure_score << std::endl;
    return this->structure_score;
}

float
EchoQualityAssessmentA4C::gain_judgment(
...
s_f_map
EchoQualityAssessmentA4C::predict(
    std::vector<cv::Mat> frames,  std::vector<float> radius, singleResult& results)
{
    ...
    if (this->structure_frames.empty())
        return s_f_map();
   // this->keyframes.push_back(5);//随便压栈一个值进去，单纯看看检测非必需项
    ...
    //results.grades.push_back(scaling_score_out);
    A4C_MAP["gain_score"] = gain_score_out;
    A4C_MAP["structure_score"] = this->structure_score;
    A4C_MAP["scaling_score"] = scaling_score_out;
    return A4C_MAP;
}
