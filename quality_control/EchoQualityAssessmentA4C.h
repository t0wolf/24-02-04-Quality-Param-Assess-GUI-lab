...
class EchoQualityAssessmentA4C
{
public:

    EchoQualityAssessmentA4C(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentA4C();
...
protected:
    ...
    // 先验知识涉及参数
    std::string viewname = "A4C";                               // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    std::vector<std::string> structures = { "HEART",
        "IAS","IVS","LA","LV","MV","RA","RV","TV" };            // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
