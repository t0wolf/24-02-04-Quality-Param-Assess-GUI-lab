...
EchoQualityAssessmentA5C::EchoQualityAssessmentA5C(EchoQualityAssessmentParams params)
{
    // -------------------------------- 先验知识参数初始化 -------------------------------- //
    this->mParams = &params;
    // 切面类型名称
    ...
    this->score_weighted["mandrel"] = 2.0f;

    //// 解剖结构
    // 常规必须项：集合内元素均需被检测到，缺一不可的解剖结构
    // 常规可选项：检测到即得分的解剖结构
    // 非常规项：  需要单独处理的解剖结构，如A4C“二尖瓣显示清晰”除各帧检测结果，还需要结合心动周期
    this->structure_score_structures.routine_must = { "AO","LV","RV","IVS" };  // 常规必须项结构
    this->structure_score_structures.routine_optional = { "LA","RA" };// 常规可选项结构
    this->structure_score_structures.unroutine_optional = { "MV", "TV" };                  // 非常规项结构
    this->structure_score_structures.unroutine_optional_params = { 0.75f, 0.45f };      // 非常规项参数
    this->structure_score_weight.routine_must = 4.0f;                               // 常规必须项结构分数权重
    this->structure_score_weight.routine_optional = { 0.5f, 0.5f };        // 常规可选项结构分数权重
    this->structure_score_weight.unroutine_optional = { 0.5f, 0.5f };               // 非常规项结构分数权重
    ...
    //this->mParams->kfdetection_params.viewname = "a5c";
    //this->mParams->kfdetection_params.model_name = "keyframe_detection_a5c";
    ////this->mParams->kfdetection_params.model_dir = "../../extern/keyframe_detection/";
    //this->mParams->kfdetection_params.model_dir = "D:/Drum_Echocardiography/Echocardiography_Codes/AIserverOffline/extern/keyframe_detection/";
    //this->mParams->kfdetection_params.mMean = this->mMean;

    //this->mParams->ac_params.viewname = "a5c";
    //this->mParams->ac_params.structures = this->structures;
    //this->mParams->ac_params.classnames = this->classnames;
    //this->mParams->ac_params.ac_structures.push_back("MV");
    //this->mParams->ac_params.ac_structures.push_back("LV");
    //this->mParams->ac_params.model_name = "a5c_lv_segmentation";
    //////// update 2023.12.28
    //this->mParams->ac_params.inputSize = 512;
    ////this->mParams->ac_params.model_dir = "../../extern/angle_calculation/";
    //this->mParams->ac_params.model_dir = "D:/Drum_Echocardiography/Echocardiography_Codes/AIserverOffline/extern/angle_calculation/";
    //this->mParams->ac_params.angle_threshold = 30.0f;
    //this->mParams->ac_params.mMean = this->mMean;

    this->mParams->gain_params.viewname = "a5c";
    this->mParams->gain_params.frame_threshold = 0.15f;
    ...
    //this->mParams->gain_params.vi ew_structures = this->structures;
    ...
    initialize();
    //load_keyframe_model();                                 // 关键帧检测模型载入
    //load_angle_model();                                    // 心轴角度模型载入

}

EchoQualityAssessmentA5C::~EchoQualityAssessmentA5C()
{
    ...
    //delete this->keyframe_detection;
    //delete this->angle_calculation;
}
void
EchoQualityAssessmentA5C::initialize() {
...
void
EchoQualityAssessmentA5C::load_structure_model()
{
...
//void
//EchoQualityAssessmentA5C::load_keyframe_model()
//{
//    //this->mParams->kfdetection_params.viewname = "a4c";
//    //this->mParams->kfdetection_params.model_name = "keyframe_detection";
//    //this->mParams->kfdetection_params.model_dir = "../../extern/keyframe_detection/";
//    //this->mParams->kfdetection_params.mMean = this->mMean;
//
//    this->keyframe_detection = new KeyframeDetection(this->mParams->kfdetection_params);
//}

void
EchoQualityAssessmentA5C::load_gain_model()
{
...
//void
//EchoQualityAssessmentA5C::load_angle_model()
//{
//    //this->mParams->ac_params.viewname = "a4c";
//    //this->mParams->ac_params.structures = this->structures;
//    //this->mParams->ac_params.classnames = this->classnames;
//    //this->mParams->ac_params.mv_name = "MV";
//    //this->mParams->ac_params.lv_name = "LV";
//    //this->mParams->ac_params.model_name = "a4c_lv_segmentation";
//    //this->mParams->ac_params.model_dir = "../../extern/angle_calculation/";
//    //this->mParams->ac_params.angle_threshold = 60.0f;
//    //this->mParams->ac_params.mMean = this->mMean;
//
//    this->angle_calculation = new AngleCalculationA4C(this->mParams->ac_params);
//}



float EchoQualityAssessmentA5C::structure_judgment(s_frames structure_frames,
    int frame_nums,
    std::vector<float>& grades)
{
    ...
    if (!routine_optional.empty())
    {
        for (int i = 0; i < routine_optional.size(); i++)
        {
            ...
            if (structure != "RA") {
                ...
                int structure_idx = this->structure_idx_mapping[structure];
                int temp = int(std::accumulate(structure_preds[structure].begin(), structure_preds[structure].end(), 0.0));
                float pred_structure = temp >= (0.8 * frame_nums) ? 1.0 : 0.0;
                pred_structure *= this->structure_score_weight.routine_optional[i];
                this->structure_scores.routine_optional.push_back(pred_structure);
                grades.push_back(pred_structure);
            }
    ...
    if (!unroutine_optional.empty())
    {
        for (int i = 0; i < unroutine_optional.size(); i++)
        {
            ...
            std::vector<int>::const_iterator preds_end = structure_preds_sub.end();
            if (structure_preds_sub.empty()) {
                temp_grades.push_back(0.0);
                continue;
            }
            ...
            for (int j = 1; j < keyframes.size(); j++)
            {
                ...
                subpreds.assign(preds_first , preds_end);
            ...
            //                if (frame_nums - keyframes[keyframes.size() - 1] > 10) {
            //    int keyframe_s = keyframes[keyframes.size() - 1];
            //    int keyframe_e = frame_nums - 1;
            //    std::vector<int> subpreds;
            ...
            temp_grades.push_back(pred_structure);
           // }
      /*      else {
                float pred_structure = 0.0;
                int keyframe_s = 0;
                int keyframe_e = frame_nums;
                std::vector<int> subpreds;
                subpreds.assign(preds_first + keyframe_s, preds_first + keyframe_e);
                float temp = float(std::accumulate(subpreds.begin(), subpreds.end(), 0.0));
                if (temp >= subpreds.size() * unroutine_optional_params[i])
    ...
    grades.insert(iter, temp_grades[0]);
    grades.push_back(temp_grades[1]);
...
float
EchoQualityAssessmentA5C::scaling_judgment(s_frames structure_frames,
...
float
EchoQualityAssessmentA5C::gain_judgment(
...
//
//float
//EchoQualityAssessmentA5C::mandrel_judgment(
//    f_structures frame_structures,
//    std::vector<cv::Mat> frames,
//    std::vector<int> keyframes)
//{
//    this->mandrel_score = float(this->angle_calculation->predict(frames, keyframes, frame_structures));
//
//    return this->mandrel_score;
//}

//单帧图像进行目标检测预测
float
EchoQualityAssessmentA5C::structure_detection_predict(
    cv::Mat frame)
{
...
s_f_map
EchoQualityAssessmentA5C::predict(
    std::vector<cv::Mat> frames,std::vector<float> radius, singleResult& results)
{
  ...
  //  this->keyframes = this->keyframe_detection->predict(frames);
    //在EchoQualityAssessmentA5C这个类里面申明了    KeyframeDetection* keyframe_detection;                      // 关键帧检测类
    ...
    //// 心轴角度
    //this->mandrel_score = this->mandrel_judgment(this->frame_structures, frames, this->keyframes);
    //float mandrel_score_out = this->mandrel_score * this->score_weighted["mandrel"];
    //results.grades.push_back(mandrel_score_out);

    // 解剖结构得分
    this->structure_score = this->structure_judgment(this->structure_frames, int(frames.size()), results.grades);
    s_f_map result_map_a5c;
    result_map_a5c["gain_score"] = gain_score_out;
    result_map_a5c["scaling_score"] = scaling_score_out;
    result_map_a5c["structure_score"] = this->structure_score;
    return result_map_a5c;
}
