...
class EchoQualityAssessmentPSAXA
{
public:

    EchoQualityAssessmentPSAXA(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentPSAXA();
    ...
    float total_score_th = 4.0f;                                // 图像各子项目总分标准切面判定阈值
...
protected:
    ...
    IntegrityClassification* integrity_classification;

    // 先验知识涉及参数
    std::string viewname = "PSAXA";                               // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    std::vector<std::string> structures = { "HEART", "RV" };
    ...
    std::vector<float> scaling_range = { 1.0f / 3.0f, 2.0f / 3.0f };  // 相应切面缩放比例的合理范围
