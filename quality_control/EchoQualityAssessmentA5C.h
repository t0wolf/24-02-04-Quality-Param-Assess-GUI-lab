...
class EchoQualityAssessmentA5C
{
public:

    EchoQualityAssessmentA5C(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentA5C();
    ...
    float structure_detection_predict(cv::Mat frames);//单帧图像目标检测？
    ...
    float mandrel_score = 0.0f;                                 // 心轴角度得分
...
protected:
    ...
    //void load_keyframe_model();                                 // 关键帧检测模型载入
    //void load_angle_model();                                    // 心轴角度模型载入
    ...
    //// 心轴角度得分
    //float mandrel_judgment(f_structures frame_structures,
    //    std::vector<cv::Mat> frames,
    ...
    //AngleCalculationA4C* angle_calculation;                     // 心轴角度计算类


    // 先验知识涉及参数
    std::string viewname = "A5C";                               // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    //std::vector<std::string> structures = { "HEART",
    //    "AO","IVS","LA","LV","MV","RA","RV","TV" };            // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
    std::vector<std::string> structures = { "HEART",
        "LA","LV","MV","RA","RV","TV", "AO", "IVS"};            // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
