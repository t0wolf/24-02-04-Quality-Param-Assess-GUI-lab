...
class EchoQualityAssessmentPSAXPM
{
public:

    EchoQualityAssessmentPSAXPM(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentPSAXPM();
...
protected:
    ...
    // 先验知识涉及参数
    std::string viewname = "PSAXPM";                            // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    //std::vector<std::string> structures = {"HEART","ALPap",
    //    "IVS", "LVante", "LVinpo", "LVside", "PMPap", "RV"};    // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
    std::vector<std::string> structures = { "HEART","IVS","RV","LVinpo","ALPap","PMPap",
     "LVante", "LVside" };    // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
