...
class EchoQualityAssessmentPSAXGV
{
public:

    EchoQualityAssessmentPSAXGV(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentPSAXGV();
...
protected:
    ...
    // 先验知识涉及参数
    std::string viewname = "PSAXGV";                            // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    std::vector<std::string> structures = { "HEART",
        "A", "AV", "LA", "PA", "RA", "RVOT" };                  // 相应切面所包含的解剖结构（建议标注时"HEART"放第一个）
    ...
    std::vector<int> keyframes;                                 // 关键帧索引
