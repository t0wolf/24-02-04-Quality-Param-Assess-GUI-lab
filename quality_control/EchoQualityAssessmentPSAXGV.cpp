...
EchoQualityAssessmentPSAXGV::EchoQualityAssessmentPSAXGV(EchoQualityAssessmentParams params)
{
    // -------------------------------- 先验知识参数初始化 -------------------------------- //
    this->mParams = &params;
    // 切面类型名称
    ...
    this->structure_score_structures.routine_must = { "A", "LA", "RVOT" };  // 常规必须项结构
    this->structure_score_structures.routine_optional = { "AV", "PA", "RA" }; // 常规可选项结构
    ...
    this->structure_score_weight.routine_optional = { 1.0f, 1.0f, 1.0f };   // 常规可选项结构分数权重
    ...
    // -------------------------------- 载入各模型 -------------------------------- //
    this->mParams->sdetection_params.viewname = "psaxgv";
    this->mParams->sdetection_params.structures = this->structures;
    this->mParams->sdetection_params.classnames = this->classnames;
    this->mParams->sdetection_params.model_name = "structure_detection_psaxgv";
    ...
    this->mParams->sdetection_params.frame_threshold = { 1,1,1,1,5,1,1,1,1 };
    this->mParams->sdetection_params.structures = this->structures;
    this->mParams->sdetection_params.mMean = this->mMean;

    this->mParams->gain_params.viewname = "psaxgv";
    this->mParams->gain_params.frame_threshold = 0.07f;
    ...
    ////this->mParams->gain_params.gain_structures = { "PA" };
    //this->mParams->gain_params.gain_structures = { "HEART" };
    //this->mParams->gain_params.gain_threshold = { 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f };
    //this->mParams->gain_params.frame_threshold = { 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f };
...
EchoQualityAssessmentPSAXGV::~EchoQualityAssessmentPSAXGV()
{
...
void
EchoQualityAssessmentPSAXGV::load_structure_model()
{
...
void
EchoQualityAssessmentPSAXGV::load_gain_model()
{
...
float EchoQualityAssessmentPSAXGV::structure_judgment(s_frames structure_frames,
    ...
    std::vector<float>& grades)
{
    ...
    routine_must_sorce *= this->structure_score_weight.routine_must;
    this->structure_scores.routine_must = routine_must_sorce;
    float temp_score = routine_must_sorce;
...
float
EchoQualityAssessmentPSAXGV::scaling_judgment(s_frames structure_frames,
...
float
EchoQualityAssessmentPSAXGV::gain_judgment(
...
s_f_map
EchoQualityAssessmentPSAXGV::predict(
    std::vector<cv::Mat> frames,  std::vector<float> radius, singleResult& results)
{
    ...
    s_f_map PSAXGV_MAP;
    PSAXGV_MAP["gain_score"] = this->gain_score;
    PSAXGV_MAP["structure_score"] = this->structure_score;
    PSAXGV_MAP["scaling_score"] =this-> scaling_score;
    return PSAXGV_MAP;
}
