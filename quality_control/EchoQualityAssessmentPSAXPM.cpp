...
EchoQualityAssessmentPSAXPM::EchoQualityAssessmentPSAXPM(EchoQualityAssessmentParams params)
{
    // -------------------------------- 先验知识参数初始化 -------------------------------- //
    this->mParams = &params;
    // 切面类型名称
    ...
    this->structure_score_structures.routine_must = { "IVS", "LVinpo"};   // 常规必须项结构，必须每个都有才得5分
    this->structure_score_structures.routine_optional = { "LVante", "LVside"};  // 常规可选项结构
    //this->structure_score_structures.routine_or = { "ALPap", "PMPap" };       // 常规必须项中，有一个即可得分的结构
    //this->structure_score_structures.routine_and = { "ALPap", "PMPap" };      // 常规必须都有才得分的可选项结构
    this->structure_score_structures.unroutine_optional = { "ALPap", "PMPap" }; // 非常规项结构，or后乘以routine_must，and后作为自身分值
    ...
    // -------------------------------- 载入各模型 -------------------------------- //
    this->mParams->sdetection_params.viewname = "psaxpm";
    this->mParams->sdetection_params.structures = this->structures;
    this->mParams->sdetection_params.classnames = this->classnames;
    this->mParams->sdetection_params.model_name = "structure_detection_psaxpm";
   // this->mParams->sdetection_params.model_dir = "../../extern/structure_detection/";
    ...
    this->mParams->sdetection_params.score_threshold = { 0.5f, 0.5f, 0.5f, 0.01f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f }; //+1,考虑背景类,LVante检测效果较差，降低了阈值
    this->mParams->sdetection_params.frame_threshold = { 1,1,1,1,1,1,1,1,1 };
    this->mParams->sdetection_params.structures = this->structures;
    this->mParams->sdetection_params.mMean = this->mMean;

    this->mParams->gain_params.viewname = "psaxpm";
    ...
    //this->mParams->gain_params.gain_structures = { "IVS" };
...
EchoQualityAssessmentPSAXPM::~EchoQualityAssessmentPSAXPM()
{
...
void
EchoQualityAssessmentPSAXPM::load_structure_model()
{
...
void
EchoQualityAssessmentPSAXPM::load_gain_model()
{
...
float EchoQualityAssessmentPSAXPM::structure_judgment(s_frames structure_frames,
    ...
    std::vector<float>& grades)
{
    ...
    // 非常规结构单独统计, { "ALPap", "PMPap" }or后乘以routine_must，and后作为自身分值
    ...
    float count_flag = 0.0;
    for (std::string structure : unroutine_optional)
    {
        ...
        count_flag += pred_structure;
    }
    float or_flag = size_t(count_flag) > 0 ? 1.0 : 0.0;
    float and_flag = size_t(count_flag) == unroutine_optional.size() ? 1.0 : 0.0;
    // { "ALPap", "PMPap" }and后作为自身分值
    float unroutine_optional_sorce = and_flag * this->structure_score_weight.unroutine_optional[0];
    float temp_unroutine_optional_sorce = unroutine_optional_sorce;
    this->structure_scores.unroutine_optional.push_back(unroutine_optional_sorce);
    ...
    // { "ALPap", "PMPap" }or后乘以routine_must
    this->structure_scores.routine_must = or_flag * routine_must_sorce * this->structure_score_weight.routine_must;
    float temp_routine_must_sorce = this->structure_scores.routine_must;
    grades.push_back(temp_routine_must_sorce);
    ...
    // 调整unroutine_optional_sorce push_back的顺序
    grades.push_back(temp_unroutine_optional_sorce);
...
float
EchoQualityAssessmentPSAXPM::scaling_judgment(s_frames structure_frames,
...
float
EchoQualityAssessmentPSAXPM::gain_judgment(
...
s_f_map
EchoQualityAssessmentPSAXPM::predict(
    std::vector<cv::Mat> frames, std::vector<float> radius, singleResult& results)
{
    ...
    s_f_map PSAXPM_MAP;
    PSAXPM_MAP["gain_score"] = this->gain_score;
    PSAXPM_MAP["structure_score"] = this->structure_score;
    PSAXPM_MAP["scaling_score"] = this->scaling_score;
    return PSAXPM_MAP;
}
