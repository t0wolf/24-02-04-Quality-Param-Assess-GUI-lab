...
class EchoQualityAssessmentPSAXMV
{
public:


    EchoQualityAssessmentPSAXMV(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentPSAXMV();

    virtual s_f_map predict(std::vector<cv::Mat> frames, std::vector<float> radius, singleResult& results);

    bool predict(std::vector<cv::Mat> frames, std::vector<cv::Mat>& vDrawnFrames, cv::Mat roi_mask, std::vector<float> radius, singleResult& results);
...
protected:
    ...
    float scaling_judgment(std::vector<cv::Mat>& vFrames,
    ...
    // 判断是否LV是圆形
    std::vector<int> psaxmv_lv_judgement();
    ...
    // 先验知识涉及参数
    std::string viewname = "PSAXMV";                               // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    //std::vector<std::string> structures = { "LV", "LVB", "LVF", "LVL", "LVS", "MV", "RV", "HEART" };
    //std::vector<std::string> structures = { "HEART", "LV", "LVB", "LVF", "MV", "RV", };
    std::vector<std::string> structures = { "HEART", "LV", "LVF", "LVS", "LVB", "MV", "RV" };
    ...
    std::vector<float> scaling_range = { 1.0f / 2.0f, 4.0f / 5.0f };  // 相应切面缩放比例的合理范围
    ...
    std::pair<float, float> ratioRange = { 0.8, 1.2 };
};
