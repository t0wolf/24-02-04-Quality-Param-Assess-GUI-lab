...
class EchoQualityAssessmentA3C
{
public:

    EchoQualityAssessmentA3C(EchoQualityAssessmentParams params);

    virtual ~EchoQualityAssessmentA3C();
...
protected:
    ...
    // 先验知识涉及参数
    std::string viewname = "A3C";                               // 切面类型名称
    std::vector<std::string> classnames;                        // 目标检测的解剖结构表
    std::vector<std::string> structures = { "HEART",
        "LVOT","IVS","LA","LV","MV","LVPW","AO" };            // 相应切面所包含的解剖结构8个（建议标注时"HEART"放第一个）
