...
class MultiLineMaskInferer
{
public:
	MultiLineMaskInferer(std::string& sEngineFilePath);

	~MultiLineMaskInferer();
...
private:
	...
	int postProcess(float* outputEd, float* outputEs, float* outputEdMask, std::vector<std::vector<cv::Mat>>& vMasks);
...
protected:
	...
	nvinfer1::Dims4 m_outputEdMaskDims;
