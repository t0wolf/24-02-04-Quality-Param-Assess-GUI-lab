...
PLAXPoseClsInferer::PLAXPoseClsInferer(std::string& sEngineFilePath)
    : m_inputDims({ 1, 3, 320, 320 })        // 输入维度保持不变
    , m_classes(3)                          // 设置为3分类
    , m_outputViewDims({ 1, 3 })            // 输出维度改为3
{
    ...
    m_inputImageIdx = m_engine->getBindingIndex("image");
    m_outputViewIdx = m_engine->getBindingIndex("output");
...
PLAXPoseClsInferer::~PLAXPoseClsInferer()
{
    if (m_engine != nullptr)
    {
        delete m_engine;
...
int PLAXPoseClsInferer::doInference(cv::Mat& src, std::map<std::string, int>& classResults)
{
    doSingleInfer(src, classResults);
    return 1;
}

int PLAXPoseClsInferer::doSingleInfer(cv::Mat& src, std::map<std::string, int>& classResults)
{
...
int PLAXPoseClsInferer::preprocess(cv::Mat& src, float* blob)
{
    if (src.empty())
        return 0;
...
int PLAXPoseClsInferer::blobFromImage(cv::Mat& src, float* blob)
{
...
int PLAXPoseClsInferer::postProcess(float* outputView, std::map<std::string, int>& classResults)
{
    ...
    classResults.insert({ "class", maxViewIdx }); // 更改为"类"而不是"视图"
    return 1;
}