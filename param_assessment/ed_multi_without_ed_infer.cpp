...
MultiLineMaskInferer_ed::MultiLineMaskInferer_ed(std::string& sEngineFilePath)
    :m_inputDims({ 5, 3, 256, 256 })  // {1, 5, 3, 256, 256}
    , m_outputEdDims({ 1, 4, 256, 256 })
    , m_outputEdMaskDims({ 1, 4, 256, 256 })
...
int MultiLineMaskInferer_ed::doInference(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int MultiLineMaskInferer_ed::doSingleInfer(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
    void* inputMem{ nullptr };  // GPU中的内存
    void* outputEdMem{ nullptr };
    void* outputEdMaskMem{ nullptr };
    ...
    size_t outputEdMaskSize = getMemorySize(m_outputEdMaskDims, sizeof(float));
    ...
    if (cudaMalloc(&outputEdMaskMem, outputEdMaskSize) != cudaSuccess)
    {
        logger::gLogError << "ERROR: output cuda memory allocation failed, size = " << outputEdSize << " bytes" << std::endl;
        return 0;
    }
    ...
    float* outputEdMask = new float[getElementNum(m_outputEdMaskDims)];
    ...
    //void* bindings[] = { inputMem, outputEdMem, outputEdMaskMem, outputEsMem };
    void* bindings[] = { inputMem, outputEdMem, outputEdMaskMem };
    ...
    //    return 0;
    //}
    if (cudaMemcpyAsync(outputEdMask, outputEdMaskMem, outputEdMaskSize, cudaMemcpyDeviceToHost, stream) != cudaSuccess)
    {
        logger::gLogError << "ERROR: CUDA memory copy of output failed, size = " << outputEdMaskSize << " bytes" << std::endl;
        return 0;
    }

    postProcess(outputEd, outputEdMask, vMasks);
    ...
    cudaFree(outputEdMaskMem);
    cudaStreamDestroy(stream);

    return 1;
}

int MultiLineMaskInferer_ed::preprocess(std::vector<cv::Mat>& video, float* blob)
{
...
int MultiLineMaskInferer_ed::blobFromImage(std::vector<cv::Mat>& video, float* blob)
{
    cv::Mat src = video[0];
...
int MultiLineMaskInferer_ed::postProcess(float* outputEd, float* outputEdMask, std::vector<std::vector<cv::Mat>>& vMasks)
{
    vMasks.clear();
    ...
    cv::Mat edSegMask = cv::Mat::zeros(outputEdSize, CV_8UC1);
    std::vector<cv::Mat> masksEdSeg = { edSegMask };
 ...
 /*       for (int h = 0; h < height; h++)
    ...
    for (int h = 0; h < height; h++)
    {
        for (int w = 0; w < width; w++)
        {
            float maxProb = 0.0f;
            int maxProbIdx = 0;
            for (int c = 0; c < m_segClasses; c++)
            {
                float currProb = outputEdMask[c * height * width + h * width + w];
                if (currProb > maxProb)
                {
                    maxProb = currProb;
                    maxProbIdx = c;
            ...
            edSegMask.at<uchar>(h, w) = maxProbIdx;

            // for test
            /*if ((maxProbIdx == 1) || (maxProbIdx == 2))
                edSegMask.at<uchar>(h, w) = 255;*/
        }
    }

    /*cv::imshow("test", masksEdSeg[0]);
    cv::waitKey(0);*/

    vMasks = { masksEd, masksEdSeg };
