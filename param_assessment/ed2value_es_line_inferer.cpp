...
DetachTwoLineInferer::DetachTwoLineInferer(std::string& sEngineFilePath)
    :m_inputDims({ 5, 3, 256, 256 })  // {1, 5, 3, 256, 256}
    , m_outputEdDims({ 1, 2, 256, 256 })
...
DetachTwoLineInferer::~DetachTwoLineInferer()
{
    if (m_engine != nullptr)
    {
        delete m_engine;
...
int DetachTwoLineInferer::doInference(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int DetachTwoLineInferer::doSingleInfer(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int DetachTwoLineInferer::preprocess(std::vector<cv::Mat>& video, float* blob)
{
...
int DetachTwoLineInferer::blobFromImage(std::vector<cv::Mat>& video, float* blob)
{
    cv::Mat src = video[0];
...
int DetachTwoLineInferer::postProcess(float* outputEd, float* outputEs, std::vector<std::vector<cv::Mat>>& vMasks)
{
    vMasks.clear();
