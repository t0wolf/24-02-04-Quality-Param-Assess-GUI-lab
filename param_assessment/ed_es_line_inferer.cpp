...
LineInferer::LineInferer(std::string& sEngineFilePath)
	:m_inputDims({5, 3, 256, 256})  // {1, 5, 3, 256, 256}
	, m_outputEdDims({1, 6, 256, 256})
...
int LineInferer::doInference(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int LineInferer::doSingleInfer(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int LineInferer::preprocess(std::vector<cv::Mat>& video, float* blob)
{
...
int LineInferer::blobFromImage(std::vector<cv::Mat>& video, float* blob)
{
    cv::Mat src = video[0];
...
int LineInferer::postProcess(float* outputEd, float* outputEs, std::vector<std::vector<cv::Mat>>& vMasks)
{   
    vMasks.clear();
