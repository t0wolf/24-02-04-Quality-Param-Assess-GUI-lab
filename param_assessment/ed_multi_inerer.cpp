...
MultiLineMaskInferer::MultiLineMaskInferer(std::string& sEngineFilePath)
    :m_inputDims({ 5, 3, 256, 256 })  // {1, 5, 3, 256, 256}
    , m_outputEdDims({ 1, 4, 256, 256 })
    , m_outputEdMaskDims({ 1, 3, 256, 256 })
...
MultiLineMaskInferer::~MultiLineMaskInferer()
{
    if (m_engine != nullptr)
    {
        delete m_engine;
...
int MultiLineMaskInferer::doInference(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
...
int MultiLineMaskInferer::doSingleInfer(std::vector<cv::Mat>& video, std::vector<std::vector<cv::Mat>>& vMasks)
{
    ...
    postProcess(outputEd, outputEs, outputEdMask, vMasks);
...
int MultiLineMaskInferer::preprocess(std::vector<cv::Mat>& video, float* blob)
{
...
int MultiLineMaskInferer::blobFromImage(std::vector<cv::Mat>& video, float* blob)
{
    cv::Mat src = video[0];
...
int MultiLineMaskInferer::postProcess(float* outputEd, float* outputEs, float* outputEdMask, std::vector<std::vector<cv::Mat>>& vMasks)
{
    vMasks.clear();
    ...
    vMasks = { masksEd, masksEs, masksEdSeg };
