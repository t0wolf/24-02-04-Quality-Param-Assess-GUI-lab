...
class MultiLineMaskInferer_ed
{
public:
	MultiLineMaskInferer_ed(std::string& sEngineFilePath);

	~MultiLineMaskInferer_ed();
...
private:
	...
	int postProcess(float* outputEd,  float* outputEdMask, std::vector<std::vector<cv::Mat>>& vMasks);
